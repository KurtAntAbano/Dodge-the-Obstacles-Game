...

{
  ...
  if (gameMode ==0) {
    ...
    //image(playerImage, (width/2-playerImage.width), (height/2-playerImage.height));
...
void keyReleased() {
  ...
  if ((key == 'w')&&(gameMode == 1)) {  // && gameMode== 1 ensures we can only restart the game after a collision
    ...
    for (int i = 0; i < movingArray.length; i++ ) {
      movingArray[i] = new movingObstacle(color1);
    }
...
void displayText()
{
  if (gameMode == 0)
  {
    ...
    text("Total score:" + score, width/2, height/2 + 40);
    textSize(15);
